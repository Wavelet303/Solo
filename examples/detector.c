...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    while(get_current_batch(net) < net->max_batches){
        ...
        layer out = get_network_output_layer(net); // mindspree
        printf("out.h: %d \n", out.h);
        printf("out.w: %d \n", out.w);
        printf("out.c: %d \n", out.c);
        printf("out.out_h: %d \n", out.out_h);
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        ...
        int nboxes = 0;
	printf("1 Threshold:%f \n", hier_thresh);
        detection *dets = get_network_boxes(net, im.w, im.h, thresh, hier_thresh, 0, 1, &nboxes);

        //printf("%d\n", nboxes);
        //if (nms) do_nms_obj(boxes, probs, l.w*l.h*l.n, l.classes, nms);
        //if (nms) do_nms_sort(dets, nboxes, l.classes, nms); //mindspree
        draw_detections(im, dets, nboxes, thresh, names, alphabet, l.classes);  // This prints detection result
